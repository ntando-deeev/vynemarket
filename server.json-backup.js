...
const REEL_LIKES_FILE    = path.join(DATA_DIR, 'reel_likes.json');
const REEL_COMMENTS_FILE = path.join(DATA_DIR, 'reel_comments.json');
...
// ════════════════════════════════════════════════════════
//  REELS — User-posted short videos (any logged-in user)
// ════════════════════════════════════════════════════════
const reelStorage = multer.diskStorage({
  destination:(req,file,cb)=>{ const d=path.join(UPLOADS_DIR,'reels'); fs.mkdirSync(d,{recursive:true}); cb(null,d); },
  filename:(req,file,cb)=>cb(null, uuidv4()+path.extname(file.originalname))
});
...
// POST /api/reels/post — upload a new reel
app.post('/api/reels/post', authMiddleware, reelUpload.single('video'), (req,res)=>{
  try{
    if(!req.file) return res.status(400).json({error:'No video file provided.'});
    const { caption='', listingId='' } = req.body;
    const reels = readJSON(REELS_FILE);
    const users = readJSON(USERS_FILE);
    ...
    reels.unshift(reel);
    writeJSON(REELS_FILE, reels);
    pushNotification(req.user.id,'reel_posted',{ reelId:reel.id, caption:reel.caption });
    res.json({ success:true, reel });
  }catch(err){ console.error(err); res.status(500).json({error:'Server error'}); }
});

// GET /api/reels/feed — paginated feed of all user reels
app.get('/api/reels/feed', optionalAuth, (req,res)=>{
  const limit  = Math.min(parseInt(req.query.limit)||10,50);
  const offset = parseInt(req.query.offset)||0;
  const reels  = readJSON(REELS_FILE);
  const likes  = readJSON(REEL_LIKES_FILE);
  const myId   = req.user?.id||null;
  const slice  = reels.slice(offset, offset+limit).map(r=>({
    ...r,
    liked: myId ? likes.some(l=>l.reelId===r.id&&l.userId===myId) : false
  }));
  res.json({ total:reels.length, reels:slice });
});

// GET /api/reels/user/:userId — reels by a specific user
app.get('/api/reels/user/:userId', optionalAuth, (req,res)=>{
  const reels = readJSON(REELS_FILE).filter(r=>r.authorId===req.params.userId);
  res.json({ total:reels.length, reels });
});

// PUT /api/reels/:id/view — increment view count
app.put('/api/reels/:id/view', (req,res)=>{
  const reels = readJSON(REELS_FILE);
  const idx = reels.findIndex(r=>r.id===req.params.id);
  if(idx===-1) return res.status(404).json({error:'Not found'});
  reels[idx].views=(reels[idx].views||0)+1;
  writeJSON(REELS_FILE,reels);
  res.json({success:true, views:reels[idx].views});
});

// POST /api/reels/:id/like — toggle like
app.post('/api/reels/:id/like', authMiddleware, (req,res)=>{
  const reels = readJSON(REELS_FILE);
  const idx = reels.findIndex(r=>r.id===req.params.id);
  if(idx===-1) return res.status(404).json({error:'Not found'});
  const likes = readJSON(REEL_LIKES_FILE);
  const likeIdx = likes.findIndex(l=>l.reelId===req.params.id&&l.userId===req.user.id);
  let liked;
  if(likeIdx===-1){
    likes.push({id:uuidv4(), reelId:req.params.id, userId:req.user.id, createdAt:new Date().toISOString()});
    reels[idx].likeCount=(reels[idx].likeCount||0)+1;
    liked=true;
    if(reels[idx].authorId!==req.user.id){
      pushNotification(reels[idx].authorId,'reel_like',{reelId:req.params.id,likerName:req.user.name||'Someone'});
    }
  } else {
    likes.splice(likeIdx,1);
    reels[idx].likeCount=Math.max(0,(reels[idx].likeCount||1)-1);
    liked=false;
  }
  writeJSON(REEL_LIKES_FILE,likes);
  writeJSON(REELS_FILE,reels);
  io.emit('reel_like',{reelId:req.params.id, likeCount:reels[idx].likeCount});
  res.json({success:true, liked, likeCount:reels[idx].likeCount});
});

// GET /api/reels/:id/comments — get comments for a reel
app.get('/api/reels/:id/comments', (req,res)=>{
  const comments = readJSON(REEL_COMMENTS_FILE).filter(c=>c.reelId===req.params.id);
  res.json(comments);
});

// POST /api/reels/:id/comments — add a comment
app.post('/api/reels/:id/comments', authMiddleware, (req,res)=>{
  const { text='' } = req.body;
  ...
  const author = users.find(u=>u.id===req.user.id);
  const comment = {
    id:uuidv4(), reelId:req.params.id,
    authorId:req.user.id, authorName:author?.name||'User',
    authorAvatar:author?.avatar||null,
    text:text.trim().slice(0,500),
    createdAt:new Date().toISOString()
  };
  const comments = readJSON(REEL_COMMENTS_FILE);
  comments.unshift(comment);
  writeJSON(REEL_COMMENTS_FILE, comments);
  reels[idx].commentCount=(reels[idx].commentCount||0)+1;
  writeJSON(REELS_FILE,reels);
  if(reels[idx].authorId!==req.user.id){
    pushNotification(reels[idx].authorId,'reel_comment',{reelId:req.params.id, commenterName:author?.name||'Someone', text:comment.text.slice(0,80)});
  }
  io.emit('reel_comment',{reelId:req.params.id, comment, commentCount:reels[idx].commentCount});
  res.json({success:true, comment});
});

// DELETE /api/reels/:id — delete own reel
app.delete('/api/reels/:id', authMiddleware, (req,res)=>{
  const reels = readJSON(REELS_FILE);
  const idx = reels.findIndex(r=>r.id===req.params.id&&r.authorId===req.user.id);
